...
void test_ex_mkfs_parse_inodes_arg() {
    ...
    char *invalid_inodes_args[] = {
        "progname", "--inodes", "abcde", NULL
    };
...
void test_ex_mkfs_parse_size_arg() {
    ...
    char *invalid_size_args[] = {
        "progname", "--size", "-0x10000000", NULL
    };

    int rv = ex_mkfs_parse_options(&params, 3, invalid_size_args);
    assert(rv == EX_MKFS_OPTION_PARSE_ERROR);

    char *invalid_size_args1[] = {
        "progname", "--size", "0x100000000000000000000000", NULL
    };

    rv = ex_mkfs_parse_options(&params, 3, invalid_size_args1);
    assert(rv == EX_MKFS_OPTION_PARSE_ERROR);

    char *valid_size_args[] = {
        "progname", "--size", "0x10000000000", NULL
    };
...
void test_ex_mkfs_parse_log_level() {
    ...
    char *invalid_log_level[] = {
        "progname", "--log-level", "abcde"
    };

    int rv = ex_mkfs_parse_options(&params, 3, invalid_log_level);
    assert(rv == EX_MKFS_OPTION_PARSE_ERROR);

    char *valid_log_level[] = {
        "progrname", "--log-level", "error"
    };
